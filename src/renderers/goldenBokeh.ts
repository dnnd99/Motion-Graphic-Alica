...
export function renderGoldenBokeh(
  ...
  const rayCount = 12;
  const rayRotCycles = Math.max(1, Math.round(params.speed));
  for (let i = 0; i < rayCount; i++) {
    const rayAngle = (i / rayCount) * Math.PI * 2 + (progress * rayRotCycles * (2 * Math.PI / rayCount));
    const rayWidth = 0.15 + sinLoop(progress, 2, i * 0.2) * 0.05;
  ...
  for (let i = 0; i < particleCount; i++) {
    const seed = i * 997;
    const pProgress = (progress * speedCycles + (seed % 100) / 100) % 1;
    ...
    const shimmer = Math.pow(sinLoop(pProgress, 4 + (i % 2), i), 2);
